...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            if (!firefox)
            	container.append($("<div class='ProgressBar' style='display:none;border:solid 1px blue;width:100px;'> <div style='background-color:blue;width:0%'>&nbsp;</div> </div>"));
            container.append($("<span id='" + id + "_completedMessage'/>"));
            container.append($("<span id='" + id + "_uploading'><span>Uploading...</span> <input type='button' value='Cancel'/></span>"));
            ...
            var defaults = {
								flash_url : "jquery/jqueryUpload/swfupload.swf",
								flash9_url : "jquery/jqueryUpload/swfupload_fp9.swf",
								upload_url: "upload.php",
								post_params: {"SESSIONID" : "session id"},
								...
								// Button settings
								button_image_url: "jquery/jqueryUpload/blankButton.png",
								button_width: width,
								button_height: height,
                button_placeholder_id: id + "_swf",
                button_text: "<font face='Arial' size='13pt'>Select File ...</span>",/*
                ...
                upload_complete_handler: function() {
                    ...
                    if ($("input[name$=_filename]", container).val() != "") { // Success 
                    	if (!firefox)
	                    	$("div.ProgressBar div", container).animate({ width: "100%" }, 
	                    		{ duration: "fast", queue: false, 
	                    			complete: function() {
					                    ...
					                    if (options.onSuccess)
					                    	options.onSuccess($("input[name$=_guid]", container).val());
	                    			
	                    			} });
                    	$("span[id$=_uploading] span", container).html("100%");
                		} else // Fail
                        clearup();
                ...
                upload_progress_handler: function(file, bytes, total) {
                    var percent = Math.ceil(100 * bytes / total);
                    $("span[id$=_uploading] span", container).html(percent + "%");
                    if (!firefox)
